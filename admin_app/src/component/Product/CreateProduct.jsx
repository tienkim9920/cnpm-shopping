...
function CreateProduct(props) {
    ...
    const addProduct = async () => {
        ...
        await axios.post(
            "http://localhost:8000/api/admin/product/create",
            formData
        ).then(response => {
            const msg = response.data.msg
            if (msg === "Bạn đã thêm thành công") {
                setName('');
                setPrice('');
                setDescription('');
                setNumber('')
            ...
            setValidationMsg({ api: msg })
        }).catch(error => {
            setValidationMsg({ api: error })
        })
    }
                            ...
                            <div className="card-body">
                                ...
                                <form action="/products/create" method="POST" encType="multipart/form-data">
                                    ...
                                    <div className="form-group w-50">
                                        ...
                                        <input type="number" className="form-control" id="number" name="number" value={number} onChange={(e) => setNumber(e.target.value)} required />
                                    ...
                                    <button type="button" className="btn btn-primary" onClick={handleCreate}>Create Product</button>
                                </form>
                            </div>
                        </div>
                    </div>
